...
- Markdown в конспекте
  CodeMirror 6 (я бы начал с него) @CodeMirror6

Почему: лёгкий, модульный, отлично встраивается, line numbers + markdown режим.

Vue 3 обёртки:

vue-codemirror (CM6)

или напрямую CM6 через @codemirror/\*

Фичи, которые легко включаются и давай включим:

номера строк (удалим че было), подсветка Markdown

поиск/замена, множественные курсоры

автозакрытие скобок/кавычек, history (undo/redo)

сворачивание (fold), подсказки

read-only режим - радиокнопка просмотра с глазиком, как на гит хабе

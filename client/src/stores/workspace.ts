...
export const useWorkspaceStore = defineStore('workspace', () => {
  ...
  async function updateCard(id: string, input: UpdateCardInput) {
    ...
    if (currentWorkspace.value?.backlog) {
      currentWorkspace.value = {
        ...currentWorkspace.value,
        backlog:
          updateInList(currentWorkspace.value.backlog, input as unknown as Partial<CardGql>) ??
          currentWorkspace.value.backlog,
      };
    }
    ...
    try {
      ...
      if (currentWorkspace.value?.backlog) {
        currentWorkspace.value = {
          ...currentWorkspace.value,
          backlog:
            updateInList(currentWorkspace.value.backlog, updated) ?? currentWorkspace.value.backlog,
        };
      }
  ...
  async function undoDeleteCard() {
    ...
    try {
      ...
      // Синхронизируем с сервером чтобы получить правильный порядок
      if (currentWorkspace.value) {
  ...
  async function createTool(input: CreateToolInput) {
    try {
      ...
      if (
        currentWorkspace.value &&
        (input.workspaceId === currentWorkspace.value.id ||
          (!input.workspaceId && !currentWorkspace.value.id)) // !id check is tricky, usually workspace has id
      ) {

  ...
  <div class="page-container">
    ...
    <template v-else>
      ...
      <template v-else-if="workspaceStore.workspaces.length > 0">
        <!-- ── My workspaces: Pinned ─────────────────────────────────── -->
        <template v-if="pinnedOwn.length > 0">
          ...
          <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <div
              v-for="workspace in pinnedOwn"
              :key="workspace.id"
              class="card-hover text-left p-5 group relative cursor-pointer border-l-3 border-transparent"
            ...
            >
              ...
              <h3
                ...
                class="font-semibold text-fg mb-1.5 truncate text-base"
              >
                {{ workspace.title }}
              </h3>
        ...
        <template v-if="unpinnedOwn.length > 0">
          <h2
            v-if="pinnedOwn.length > 0 || sharedWorkspaces.length > 0"
          ...
          <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <div
              v-for="workspace in unpinnedOwn"
              :key="workspace.id"
              class="card-hover text-left p-5 group relative cursor-pointer border-l-3 border-transparent"
              @click="navigateToWorkspace(workspace.id)"
            >
        ...
        <!-- ── Shared workspaces ─────────────────────────────────────── -->
        <template v-if="sharedWorkspaces.length > 0">
          <hr v-if="myWorkspaces.length > 0" class="border-border my-10" />
          <h2 class="text-sm font-medium text-muted mb-3 flex items-center gap-1.5">
          ...
          <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <div
              ...
              :key="workspace.id"
              class="card-hover text-left p-5 group relative cursor-pointer border-l-3 border-primary/30"
              @click="navigateToWorkspace(workspace.id)"
            >
              <div
                class="w-10 h-10 rounded-xl flex-center mb-3 text-2xl transition-colors bg-fg/5 group-hover:bg-fg/8"
